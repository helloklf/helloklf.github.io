<template>
  <div :disabled="disabled" @touchstart="active" @click="touch" class="touch-block">
    <slot></slot>
  </div>
</template>
...
.touch-block_ripple.show {
  animation:touch-block_ripple .5s ease-out;
}
@keyframes touch-block_ripple {to {transform:scale(2);opacity:0;}}

@keyframes anm_touch-block {
  from {
    background: #fff;
  }
  to {
...
/*hover和active各有缺陷*/
/*
.touch-block:not(.disabled).active {
  background: #ccc !important;
  animation: anm_touch-block 0.2s;
}
*/
</style>
...
export default {
  ...
  methods: {
    active (e) {
      ...
      if (this.disabled || !target || !target.classList.contains('touch-block')) {
        return
      }
      let rect = target.getBoundingClientRect()
      let ripple = target.querySelector('.touch-block_ripple')
      if (!ripple) {
        ripple = document.createElement('span')
        ripple.className = 'touch-block_ripple'
